...
while i in range(0,iterations):
	...
	y=0
	starttemp=time.time()
	while y in range(0,len(template)-1):
	...
	while n in range(0,len(templates)):
		s=time.time()
		x=insertions.index([templates[n],wc])
		findtime+=time.time()-s
		s=time.time()
		templates[n]=templates[n][0]+insertions[x][1]+templates[n][1]
	...
	template=""
	startdup=time.time()
	while n in range(0,len(templates)-1):
